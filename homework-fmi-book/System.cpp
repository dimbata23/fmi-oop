...
System::System() : users(nullptr), numOfUsers(1), usersCapacity(DEFAULT_CAPACITY), numOfBlockedUsers(0)
{
...
void System::addPost(const char* issuer, const Post& post) const
{
	User* issuerPtr = getUser(issuer);
	if (!issuerPtr) {
		std::cout << USER_NOT_FOUND_MSG << issuer << std::endl;
...
{
	for (int i = 0; i < numOfUsers; ++i)
		if (!stricmp(nickname, users[i]->getNickname()))
			return users[i];
	return nullptr;
}
...
bool System::userExists(const char* nickname) const { return getUser(nickname); }
