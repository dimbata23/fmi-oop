...
Post::Post(const Post& other) { copyData(other); }
...
//int Post::getIssuerId() const { return issuerId; }
...
{
	char* temp = new (std::nothrow) char[strlen(other.content) + 1];
	if (temp) {
		content = temp;
		strcpy(content, other.content);
	} else {
		std::cerr << "Not enough memory to copy Post!\n";
	}
...
int Post::getNextID() const { return currID++; }
