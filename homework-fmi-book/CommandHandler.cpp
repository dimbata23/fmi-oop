...
static const char* commands[] = { "info", "quit", "help", "add_user", "add_moderator", 
								  "remove_user", "block", "post", "remove_post", "view_post", 
								  "view_all_posts", "rename" };
...
bool CommandHandler::operator()(System& sys, const char* command)
{
	...
	if (numOfArgs == commandArguments[0][0][0]) {
		if (!stricmp(arguments[1], commandArguments[INFO][1])) {
			sys.printInfo();
		} else if (!stricmp(arguments[1], commandArguments[QUIT][1])) {
			clearMemory();
			return true;
		} else if (!stricmp(arguments[1], commandArguments[HELP][1])) {
			printHelp();
		} else {
	...
	if (numOfArgs > 1) {
		if (!stricmp(arguments[2], commandArguments[ADD_USER][2])) {
			if (numOfArgs == commandArguments[ADD_USER][0][0])
				sys.addUser(arguments[1], User(arguments[3], atoi(arguments[4])));
			else
				printUsage(ADD_USER);
		} else if (!stricmp(arguments[2], commandArguments[ADD_MODERATOR][2])) {
			...
			if (numOfArgs > 2) {
				...
				} else if (!stricmp(arguments[3], "text")) {
					if (numOfArgs > 3) {
		...
		} else if (!stricmp(arguments[2], commandArguments[REMOVE_POST][2])) {
			if (numOfArgs == 3)
				sys.removePost(arguments[1], atoi(arguments[3]));
			else
				printUsage(REMOVE_POST);
		} else if (!stricmp(arguments[2], commandArguments[VIEW_POST][2])) {
			if (numOfArgs == 3)
				sys.viewPost(arguments[1], atoi(arguments[3]));
			else
				printUsage(VIEW_POST);
		} else if (!stricmp(arguments[2], commandArguments[VIEW_ALL_POSTS][2])) {
			if (numOfArgs == 3)
				sys.viewAllPostsBy(arguments[1], arguments[3]);
			else
				printUsage(VIEW_ALL_POSTS);
		} else if (!stricmp(arguments[2], commandArguments[RENAME][2])) {
			if (numOfArgs == 3)
