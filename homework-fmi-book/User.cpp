...
User::User(const User& other) { copyData(other); }
...
User::~User() { deleteData(); }

bool User::toggleBlock() { return blocked = !blocked; }
...
const char* User::getNickname() const { return nickname; }

short User::getAge() const { return age; }
...
bool User::containsPost(int ID) const { return getPost(ID); }
...
bool User::createViewFor(const char* viewer) 
{
	...
	if (!fileName) {
		std::cout << "Not enough memory.. Couldn't generate the file...\n";
		return false;
	}
...
bool User::isModerator() const { return false; }

User* User::clone() const  { return new(std::nothrow) User(*this); }
...
bool User::copyData(const User& other)
{
